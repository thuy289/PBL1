...
import posmining.example.OnigiriCount;
import posmining.utils.CSKV;
import posmining.utils.PosUtils;

public class YoungJump {
	...
	// MapReduceを実行するためのドライバ
			public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
				...
				job.setJobName("2014003");                   // ★自分の学籍番号
				...
				String outputpath = "out/YoungJump";     // ★MRの出力先
				if (args.length > 0) {
					inputpath = args[0];
				}
			...
			public static class MyMapper extends Mapper<LongWritable, Text, CSKV, CSKV> {
				protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
					...
					if (csv[PosUtils.ITEM_CATEGORY_NAME].equals("新規アイテム") == false) {
						return;
					}
					...
					context.write(new CSKV(csv[PosUtils.ITEM_NAME]), new CSKV(count));
