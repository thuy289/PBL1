...
public class RubberSalesByLocation {
	// MapReduceを実行するためのドライバ
	public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {

		// MapperクラスとReducerクラスを指定
		Job job = new Job(new Configuration());
		job.setJarByClass(RubberSalesByLocation.class);       // ★このファイルのメインクラスの名前
		...
		String outputpath = "out/RubberSalesByLocation";     // ★MRの出力先
		if (args.length > 0) {
			inputpath = args[0];
		}
	...
	public static class MyMapper extends Mapper<LongWritable, Text, CSKV, CSKV> {
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			String location = "";
			// 立地を取得
			if (csv[PosUtils.LOCATION].equals("オフィス街1")) {
				location = "1";
			} else if (csv[PosUtils.LOCATION].equals("駅前1")) {
				location = "2";
			} else if (csv[PosUtils.LOCATION].equals("ロードサイド1")) {
				location = "3";
			} else if (csv[PosUtils.LOCATION].equals("住宅街1")) {
				location = "4";
			} else {
				location = "Bad Status";
			}
			...
			context.write(new CSKV(location), new CSKV(count));
		}
	}
