...
public class TimezoneCountByAge1 {
	// MapReduceを実行するためのドライバ
		public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {

			// MapperクラスとReducerクラスを指定
			Job job = new Job(new Configuration());
			job.setJarByClass(TimezoneCountByAge1.class);   // ★このファイルのメインクラスの名前
			...
			String outputpath = "out/timezoneCountByAge1";    // ★MRの出力先
			if (args.length > 0) {
				inputpath = args[0];
			}
		 ...
		 * <ul>
		 * <li>実年でないレシートは除外
		 * <li>レシートID,時間をキーに、個数をバリューにしてemit
		 * </ul>
		 * @author 紀之
		 *
		 */
		...
		public static class MyMapper extends Mapper<LongWritable, Text, CSKV, CSKV> {
			protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
				...
				String rid_hour = csv[PosUtils.RECEIPT_ID]+","+csv[PosUtils.HOUR] ;
				context.write(new CSKV(rid_hour), new CSKV(count));
			}
		}
		 ...
		 * <ul>
		 * <li>個数の数をカウント
		 * <li>割って1にすることで1つのレシートIDがどの時間に購入しているか把握
		 * <li>購入時間をキーに、tをバリューにしてemit
		 * </ul>
		 * @author 紀之
		 *
		 */
		...
		public static class MyReducer extends Reducer<CSKV, CSKV, CSKV, CSKV> {
			protected void reduce(CSKV key, Iterable<CSKV> values, Context context) throws IOException, InterruptedException {
				...
				long t = count/n;
				String hour = key.toString().split(",")[1];

				// emit
				context.write(new CSKV(hour),new CSKV(t));
			}
		}
}
