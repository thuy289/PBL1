...
import posmining.example.OnigiriCount.MyMapper;
import posmining.example.OnigiriCount.MyReducer;
...
public class AveragePaymentBySex_2nd {
	...
	public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
		...
		String outputpath = "out/avgPaymentBySex";     // ★MRの出力先
		if (args.length > 0) {
			inputpath = args[0];
		}
	...
	public static class MyMapper extends Mapper<LongWritable, Text, CSKV, CSKV> {
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			if (csv[PosUtils.BUYER_SEX].equals(01) == false) {
				return;
			}

			String receiptId = csv[PosUtils.RECEIPT_ID];
			String payment = csv[PosUtils.ITEM_TOTAL_PRICE];

			// emitする （emitデータはCSKVオブジェクトに変換すること）
			context.write(new CSKV(receiptId), new CSKV(payment));
		}
	}
